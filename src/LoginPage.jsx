...
const LoginPage = () => {
  ...
  const validationSchema = yup.object().shape({
    username: yup
      ...
      .max(16, "username has a max-length of 16 characters")
      .trim()
      .required("jojojojo"),
    pw: yup
      .string()
  ...
  const signIn = async (data) => {
    try {
      await API.post(`${URL}/users/login`, data).then((response) => {
  ...
  return (
    <div className={cl.formCard}>
      ...
      <form onSubmit={handleSubmit(signIn)} className={cl.formCardForm}>
        <label htmlFor="username" className={cl.formCardLabel}>
          username:
        </label>
        <input
        ...
        <label htmlFor="password" className={cl.formCardLabel}>
          password:
        </label>
        <input
          className={cl.formCardInput}
